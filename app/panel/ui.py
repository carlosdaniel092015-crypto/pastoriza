...
function renderChats(){
  ...
  el.innerHTML = (canalSel===SIN_CANAL ? avisoSinCanal(lista.length) : '') + lista.map(c=>{
    ...
    const tag=(c.aprobacion==='pendiente'?'<span class="pagochip">'+(c.con_pago===false?'pedido por aprobar':'pago por verificar')+'</span>':'')
...
const GRUPOS=[{t:'Números de WhatsApp (canales)',s:'Uno por línea: número = nombre. Cada número tiene su propia pestaña arriba, con su configuración y sus conversaciones. Esta lista es común a los dos.',campos:['canales']},
  {t:'Envío y entrega',s:'Costo, días y notas de envío.',campos:['precio_envio','dias_envio','hora_corte','nota_envio','info_envio','minimo_envio']},{t:'Pagos',s:'Formas de pago, mínimo, a quién se manda a pagar y comprobante. El bot NO da las cuentas.',campos:['monto_minimo','formas_pago','contra_entrega','pago_whatsapp','msg_pago_derivar','banco1_nombre','banco1_cuenta','banco2_nombre','banco2_cuenta','titular','cedula','msg_comprobante','msg_monto_corto','msg_pedido_en_revision','msg_pago_aprobado','msg_retiro_aprobado','msg_rechazado']},{t:'Negocio',s:'Datos de la tienda.',campos:['direccion','telefono','horario_tienda','website','maps_url']},{t:'Venta por fardo (opcional)',s:'Déjalo vacío hasta confirmar cantidad por fardo y su envío mínimo.',campos:['fardo_cantidad','fardo_envio_minimo']},{t:'Mensajes del bot',s:'Notas y frases que usa el bot.',campos:['nota_botellon','nota_stock','msg_escalar']},
  {t:'Horario del bot',s:'En qué horario responde SOLO este número (24h, formato HH:MM). Vacío en cualquiera de los dos = sin restricción, siempre activo. Fuera del horario el mensaje sigue apareciendo en el panel para atenderlo a mano.',campos:['horario_activo_desde','horario_activo_hasta']}];
const LBL={canales:'Números y nombres',precio_envio:'Precio de envío',dias_envio:'Días de entrega',hora_corte:'Hora de corte',nota_envio:'Notas de envío',info_envio:'Info de envío',banco1_nombre:'Banco',banco1_cuenta:'Número de cuenta',banco2_nombre:'Banco 2',banco2_cuenta:'Número de cuenta 2',titular:'Titular',cedula:'RNC',pago_whatsapp:'WhatsApp de pagos',msg_pago_derivar:'Cuando preguntan dónde pagar',msg_comprobante:'Cuando llega el comprobante',msg_monto_corto:'Cuando el comprobante no cubre el total',msg_pedido_en_revision:'Retiro: cuando toma el pedido',msg_pago_aprobado:'Cuando TÚ apruebas el pago',msg_retiro_aprobado:'Cuando TÚ apruebas un retiro',msg_rechazado:'Cuando TÚ no apruebas',direccion:'Dirección',telefono:'Teléfono',horario_tienda:'Horario',website:'Website',maps_url:'Enlace de Maps',nota_botellon:'Nota de botellón',nota_stock:'Cuando no hay stock',msg_escalar:'Cuando pasa a un asesor',monto_minimo:'Pedido mínimo (RD$)',minimo_envio:'Mínimo para envío (por tamaño)',formas_pago:'Formas de pago',contra_entrega:'¿Pago contra entrega?',fardo_cantidad:'Unidades por fardo',fardo_envio_minimo:'Envío mínimo por fardo',horario_activo_desde:'Activo desde',horario_activo_hasta:'Activo hasta'};
const HINTS={canales:'Ej: 18099221092 = Tienda · 18294716701 = Mayorista. El nombre es sólo para verlo acá.',precio_envio:'En el chat: "El envío dentro del Gran Santo Domingo son RD$ …"',hora_corte:'Después de esa hora el pedido sale al día siguiente.',msg_comprobante:'El bot NO confirma el pago: avisa que se está verificando.',pago_whatsapp:'Número al que el bot manda a los clientes para pagar (el supervisor).',msg_pago_derivar:'El bot NO da las cuentas: esto es lo que contesta. Usá {numero} para el WhatsApp de pagos.',banco1_cuenta:'Guardada acá para tu referencia. El bot NUNCA se la da a un cliente.',msg_monto_corto:'No se crea el pedido: se le dice cuánto falta. Usá {falta} para el monto.',msg_pago_aprobado:'Se envía cuando le das «Aprobar pago». Usá {numero} para el número de pedido.',msg_pedido_en_revision:'Retiro en tienda: no hubo pago, así que no se habla de pago. Tampoco se le da el número.',msg_retiro_aprobado:'Retiro: no se le puede decir «pago verificado» a quien paga al retirar. Usá {numero}.',msg_rechazado:'Neutro a propósito: el motivo real lo explicás vos. Usá {numero} para el WhatsApp de pagos.',msg_escalar:'En el chat aparece antes de "Pasado a un asesor".',horario_activo_desde:'Ej: 19:00. Vacío = siempre activo.',horario_activo_hasta:'Ej: 05:00. Si es menor que "Activo desde", cruza la medianoche.'};
...
function pintarCfg(d){ cfgDirty=0;
  ...
  $('#cfg').innerHTML=GRUPOS.map(g=>`<div class="group"><h3>${g.t}</h3><div class="gsub">${g.s}</div><div class="grid">`+g.campos.map(k=>{const v=esc(String(d[k]!=null?d[k]:''));
    ...
    const long=['info_envio','msg_pago_derivar','msg_rechazado','msg_comprobante','msg_monto_corto','msg_pedido_en_revision','msg_pago_aprobado','msg_retiro_aprobado','msg_escalar','nota_envio','canales'].includes(k);
