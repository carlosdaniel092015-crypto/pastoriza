...
function pintarCfg(d){ cfgDirty=0;
  ...
  $('#cfg').innerHTML=GRUPOS.map(g=>`<div class="group"><h3>${g.t}</h3><div class="gsub">${g.s}</div><div class="grid">`+g.campos.map(k=>{const v=esc(String(d[k]!=null?d[k]:''));
    ...
    const long=['info_envio','msg_pago_derivar','msg_comprobante','msg_monto_corto','msg_pedido_en_revision','msg_pago_aprobado','msg_retiro_aprobado','msg_escalar','nota_envio','canales'].includes(k);
